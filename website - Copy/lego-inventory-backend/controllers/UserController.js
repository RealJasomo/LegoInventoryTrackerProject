      ...
      bcrypt.hash(req.body.password, 13, (err, hash) => {
        ...
        request.execute("insert_User", (err, _) => {
          if (err) {
            res
              .status(403)
              .json({ error: "Error creating the user, please try again" });
            console.log(err);
            return;
          }
          res.json({
    ...
    var err = errors
      .map((err) => {
        switch (err) {
          ...
          case "uppercase":
            return "The password must have at least one uppercase value.";
          case "lowercase":
            return "The password must have at least one lowercase value.";
          case "digits":
